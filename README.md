...
Faced with a graph that *looks the same in every neighbourhood*, the infra-observer is naturally led to abstract that pattern **to infinity** — to assume the substrate is a regular tessellation by a single fundamental polygon. We are not tessellating a Riemannian surface; the construction is the purely combinatorial gluing of copies of `CycleGraph[p]` parameterized by the Schläfli pair `{p, q}` (number of polygon sides; number of polygons meeting at each vertex). The flat-torus Euler relation `1/p + 1/q = 1/2` admits exactly three integer solutions — `{3, 6}`, `{4, 4}`, `{6, 3}` — and the paclet provides `TorusTessellation[{n, m}, {p, q}]` as a generator for these. Other Schläfli pairs (e.g. `{5, 4}`, `{7, 3}`) describe spherical or hyperbolic tilings that do not fit a flat torus and are rejected by the constructor with that explanation. Each output is vertex-transitive, so any local-isomorphism-invariant quantity — Wolfram-Hausdorff dimension and Wolfram-Ricci curvature based on volume growth in particular — is **pointwise constant**.

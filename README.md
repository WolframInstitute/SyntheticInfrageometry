...
The natural visualization of an ambiguous construction is therefore not a single picture but a **diffuse** picture: each vertex and edge of the substrate shaded by how many candidates pass through it. The result is the discrete-geometry analogue of a quantum-mechanical superposition — a density over the substrate rather than a single classical curve. The paclet exposes this as a project-wide design pattern via `InfraSceneHighlight[graph, multiObjects]` — a single primitive that takes a list of multi-objects (each a list of representations, optionally tagged with its own color) and renders the within-object multiplicity as edge thickness / vertex size and the cross-object overlap as color blending.

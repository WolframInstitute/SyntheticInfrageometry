...
FindInfraWalk::usage = "FindInfraWalk[graph, p1, p2, kspec] gives non-backtracking walks from p1 to p2 of length restricted by kspec (k, {k}, {kmin, kmax}, Infinity) with exactly \"Crossings\" -> c arrivals at an already-visited vertex; c = 0 (default) is the simple paths, c >= 1 is drawn at random as kinked walks with per-loop length confined to \"LoopLength\" (finite kspec required). Options \"Crossings\", \"LoopLength\", Method.";
...
WalkSingularities::usage = "WalkSingularities[graph, walk] gives the positions of each walk singularity: \"Cusp\", \"TriplePoint\", \"EdgeTangency\", double points as \"Crossing\", \"Tangency\", or \"Undefined\" (no link rotation), and \"EndpointIncidence\". Closed on InfraLoop and InfraString.";
InfraImmersedQ::usage = "InfraImmersedQ[graph, walk] tests whether walk is an immersed walk: a walk with no cusp (no backtrack).";
InfraGenericQ::usage = "InfraGenericQ[graph, walk] tests whether walk is a generic immersed walk: no cusps, no triple points, no edge or vertex tangencies, endpoints off the curve; \"Undefined\" double points are accepted.";

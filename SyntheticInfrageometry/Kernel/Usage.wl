...
FindInfraPoint::usage = "FindInfraPoint[graph] returns one InfraPoint atom drawn from the candidate pool. FindInfraPoint[graph, n] returns a List of n atoms, exactly n or $Failed; UpTo[n] returns up to n; All returns every pool vertex. The pool as a region is InfraSet @ FindInfraPoint[graph, All]; a tuple of mutually constrained points is FindInfraSimplex. Option \"From\" (\"Random\" (default), \"Center\", \"Periphery\", anchor -> spec)."

FindInfraSimplex::usage = "FindInfraSimplex[graph, n] returns every infra-simplex: a configuration of n vertices at prescribed mutual distances, each returned as a List of n InfraPoint atoms. FindInfraSimplex[graph, n, count] returns exactly count configurations or $Failed; UpTo[count] up to count; All (default) all. Options \"Distance\" (\"Max\" (default), \"Spread\", d, {dMin, dMax}), \"From\", \"MaxCliques\"."

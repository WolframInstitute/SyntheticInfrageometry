...
InfraMesoPoint::usage = "InfraMesoPoint[<|v -> m, ...|>] is a mesopoint: a finitely supported measure on the vertex set, the measure layer of the point ontology. InfraMesoPoint[{v1, ...}, {m1, ...}] is input sugar (duplicate vertices sum). Up-coercions: InfraMesoPoint[{p1, p2, ...}] of InfraPoints is the counting measure (repetition = mass); InfraMesoPoint[obj] of an InfraSet or any bundle wrapper is its vertex occupation measure. Constructed at projections (seg[[i]], FindInfraMidpoint, FindInfraShellCenter); bare vertex lists are not accepted. Accessors [\"Support\"] (an InfraSet), [\"Vertices\"], [\"Weights\"], [\"Mass\"], [\"Entropy\"], [\"First\"], and the occupation measures.";

InfraPoint::usage = "InfraPoint[v] is the atom of the point ontology: one vertex of the substrate, carrying its label verbatim (a list label like {i, j} included). A family of points is a List of atoms or an InfraSet; a measure on vertices is an InfraMesoPoint. Accessors [\"Vertex\"], [\"First\"], [\"Vertices\"], [\"Mass\"], plus graph-keyed synthetic invariants ([\"BallVolumes\", g], [\"Dimension\", g], ...) returning the bare per-radius numbers (the InfraSet form returns one row per vertex). Scene-language constructors InfraPoint[] / InfraPoint[\"Center\"] are used inside InfraScene."
FindInfraPoint::usage = "FindInfraPoint[graph] returns one InfraPoint atom drawn from the candidate pool. FindInfraPoint[graph, n] returns a List of n atoms, exactly n or $Failed; UpTo[n] returns up to n; All returns every pool vertex. With \"Distance\" the n atoms are one mutually-constrained tuple. The pool as a region is InfraSet @ FindInfraPoint[graph, All]. Options \"From\" (\"Random\" (default), \"Center\", \"Periphery\", anchor -> spec), \"Distance\" (None (default), d, {dMin, dMax}, \"Max\", \"Spread\"), \"MaxCliques\"."

FindInfraMidpoint::usage = "FindInfraMidpoint[graph, p1, p2] returns the InfraMesoPoint of the walk-centre vertices unioned over every geodesic (one vertex at even distance, a two-vertex mesopoint at odd). FindInfraMidpoint[graph, InfraSegment[{walks}]] and FindInfraMidpoint[graph, walk] use the supplied walks. Options Method (\"Metric\" (default), \"Embedding\"), \"Tolerance\".";
FindInfraGoldenSection::usage = "FindInfraGoldenSection[graph, p1, p2] returns the InfraMesoPoint at the golden index 1 + (n-1)/GoldenRatio per geodesic, unioned. FindInfraGoldenSection[graph, InfraSegment[{walks}]] and FindInfraGoldenSection[graph, walk] use the supplied walks. Options Method (\"Metric\" (default), \"Embedding\"), \"Tolerance\".";
...
SelectInfraPoint::usage = "SelectInfraPoint[graph, vertices] draws one point from the bundle under graph distance, as a List of InfraPoint atoms. SelectInfraPoint[graph, vertices, n] returns exactly n or $Failed; UpTo[n] up to n; All the whole filtered pool. Options \"From\", \"Distance\", \"MaxCliques\"; the bundle may be a vertex list, an atom list, or any set-like Infra* wrapper. Operator form SelectInfraPoint[graph, n, opts][vertices]."
...
FindInfraPath::usage = "FindInfraPath[graph, p1, p2, kspec] returns one InfraPath wrapper of walks p1 -> p2 (non-simple allowed) with length restricted by kspec (k at most k, {k} exactly k, {kmin, kmax}, Infinity); trailing count n returns exactly n realisations or $Failed, UpTo[n] up to n, All (default) all. Options Properties (\"Simple\", \"ShortestPath\", \"LongestPath\", {\"EdgeMin\", f}, {\"EdgeMax\", f}), Method (\"Exhaustive\").";
...
FindInfraLine::usage = "FindInfraLine[graph, p1, p2] returns one InfraLine wrapper of the maximal geodesic lines through p1 and p2; FindInfraLine[graph, segment] returns maximal lines containing segment; count n returns exactly n realisations or $Failed, UpTo[n] up to n, All (default) all. Options Properties ({}), Method (\"Exhaustive\" (default), \"Greedy\" (deterministic first-admissible), \"GreedyRandomPick\" (uniform over admissible neighbors each step, seed via ambient SeedRandom)), \"Maximality\" (\"Extension\" (default), \"Diameter\"), \"Direction\" (\"BothSides\" (default), \"Forward\", \"Backward\").";
FindInfraParallel::usage = "FindInfraParallel[graph, line, p] returns unary InfraLine[{line}] wrappers for parallels through p (constant distance to line); n / UpTo[n] / All controls multiplicity. Options Properties ({}), Method (\"Exhaustive\" (default), \"Greedy\", \"GreedyRandomPick\").";

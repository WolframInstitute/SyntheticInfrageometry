...
InfraTopologicalSpace::usage = "InfraTopologicalSpace[graph, top] bundles a graph with its precomputed specialization-preorder digraph top (the full preorder, i.e. BallTopologyGraph[graph, r, \"Reduced\" -> False]). InfraTopologicalSpace[graph, r] is the convenience form that builds top from r; option \"Dual\" passes through. Accessors: [\"Graph\"], [\"Topology\"]. Used to cache the topology in sweep loops like Outer[BallContinuousMapQ, ...].";
BallTopologyGraph::usage = "BallTopologyGraph[graph, r] returns the directed graph of the specialization preorder of the Alexandrov topology on V(graph) whose closed-set subbasis is the family of closed r-balls (edge q -> p iff N_r(p) is contained in N_r(q)). Option \"Reduced\" (True (default): Hasse-style transitive reduction, self-loops dropped; False: full preorder digraph with reflexive self-loops). Option \"Dual\" (False (default): primal topology; True: dual topology, i.e. ReverseGraph of the primal specialization preorder).";
BallClosure::usage = "BallClosure[graph, r, p] returns the closure of vertex p in the Alexandrov topology with closed-set subbasis the closed r-balls, as a vertex List. Option \"Dual\" (False (default): cl(p) = {q : N_r(p) subset N_r(q)}; True: cl_dual(p) = {q : N_r(q) subset N_r(p)}).";
BallContinuousMapQ::usage = "BallContinuousMapQ[g, r, h, s, map] tests whether map: V(g) -> V(h) is continuous for the r-ball topology on g and the s-ball topology on h, i.e. monotone for the specialization preorder. BallContinuousMapQ[ts1, ts2, map] accepts precomputed InfraTopologicalSpace wrappers (faster for sweeps). Option \"Dual\" (False (default): primal; True: dual topology on both sides; dual-to-dual continuity equals primal-to-primal by relabeling). map: Association, list of Rule, or callable.";

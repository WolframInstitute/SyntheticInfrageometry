...
InfraPoint::usage = "InfraPoint[{v1, ..., vk}] is the multi-realisation point (each vertex mass 1); InfraPoint[{v1, ...}, {w1, ...}] carries explicit per-vertex mass. Accessors [\"Support\"], [\"Weights\"], [\"Mass\"], [\"First\"], plus graph-keyed synthetic invariants ([\"BallVolumes\", g], [\"Dimension\", g], ...). Scene-language constructors InfraPoint[] / InfraPoint[\"Center\"] / InfraPoint[origin, dist] are used inside InfraScene.";
FindInfraPoint::usage = "FindInfraPoint[graph] returns one vertex as {InfraPoint[{v}]}. FindInfraPoint[graph, n] returns exactly n or $Failed; UpTo[n] returns up to n; All returns every vertex. Options \"From\" (\"Random\" (default), \"Center\", \"Periphery\", anchor -> spec), \"Distance\" (d, {dMin, dMax}, \"Max\", \"Spread\"), \"MaxCliques\".";
...
InfraSegment::usage = "InfraSegment[{path1, ..., pathk}] is the multi-realisation geodesic bundle; seg[[i]] is the weighted InfraPoint of the i-th position across realisations. InfraSegment[dag_Graph] is the geodesic-DAG form with accessors [\"Graph\"], [\"Vertices\"], [\"Length\"], [\"Multiplicity\"], [\"Start\"] / [\"End\"], [\"Measure\"], and [\"Realizations\", n | UpTo[n] | All] to enumerate. Scene-language constructor InfraSegment[p, q] is used inside InfraScene.";
FindInfraSegment::usage = "FindInfraSegment[graph, p1, p2] returns InfraSegment[dag], the geodesic interval DAG of all shortest paths (also with count All). FindInfraSegment[graph, p1, p2, n] / UpTo[n] returns the enumerated form (up to n unary InfraSegment[{path}] wrappers). Options Properties ({} (default); {\"EdgeMin\", f}, {\"EdgeMax\", f}, \"LongestPath\"), Method (\"Exhaustive\" (default), \"Greedy\").";
...
InfraPath::usage = "InfraPath[{walk}] is the unary form (one walk, possibly non-simple); InfraPath[{walk1, ..., walkk}] is the multi-realisation form. Inside InfraScene, InfraPath[p1, p2, ..., pk] is the step-wise scene-DSL constructor: every length-(k-1) walk {v1, ..., vk} with vi in pi and each consecutive pair (vi, v_{i+1}) a graph edge (non-simple chains kept). FindInfraPath finders return a List of unary InfraPath[{walk}] wrappers; wrap that list with InfraPath @ ... to collapse to multi via auto-flatten. Rendered like InfraSegment (sequential-edge semantics).";
FindInfraPath::usage = "FindInfraPath[graph, p1, p2] returns {InfraPath[{walk}]} for one walk (non-simple allowed). FindInfraPath[graph, p1, p2, kspec] restricts length (k | {k} | {kmin, kmax} | Infinity); trailing n / UpTo[n] / All controls multiplicity. Options Properties (\"Simple\", \"ShortestPath\", \"LongestPath\", {\"EdgeMin\", f}, {\"EdgeMax\", f}), Method (\"Exhaustive\").";
...
FindInfraShell::usage = "FindInfraShell[graph, c, r] returns {InfraShell[{levelSet}]} for the metric shell { v : d(c, v) == r } (r may be {rmin, rmax}). FindInfraShell[graph, c, r, n] returns exactly n or $Failed; UpTo[n] returns up to n; All returns all. Options Properties (\"Separating\", \"Connected\"), Method (\"Exhaustive\" (default), \"Greedy\").";
...
FindInfraCircle::usage = "FindInfraCircle[graph, c, r] returns {InfraCircle[{cycle}]} for the canonical infra-circle around c at radius r: the shortest separating cycle in the level surface (r scalar or {rmin, rmax}). FindInfraCircle[graph, c, r, n] returns exactly n or $Failed; UpTo[n] returns up to n; All returns all. Option Properties (default {\"Separating\", \"Shortest\"}).";
...
InfraPolygon::usage = "InfraPolygon[{poly}] is the unary form, where poly = {seg1, ..., segn} is a closed chain of unary InfraSegment sides; InfraPolygon[{poly1, ..., polyk}] is the multi-realisation form. Find* returns a List of unary wrappers; wrap with InfraPolygon @ ... to collapse to multi via auto-flatten. Accessors [\"Sides\"] (the InfraSegment legs per realisation), [\"Length\"] (perimeter edge count per realisation), [\"Vertices\"] (corner vertices as unary InfraPoints).";
...
InfraTriangle::usage = "InfraTriangle[{poly}] is the unary form, where poly = {seg1, seg2, seg3} is a closed chain of three unary InfraSegment sides; InfraTriangle[{poly1, ...}] is the multi-realisation form. Find* returns a List of unary wrappers; wrap with InfraTriangle @ ... to collapse to multi via auto-flatten. Accessors [\"Sides\"], [\"Length\"], [\"Vertices\"] as for InfraPolygon.";

...
ExtendSegment::usage = "ExtendSegment[graph, segment] extends segment to a maximal line whose middle part is segment, returning InfraSegment[{line, ...}]. ExtendSegment[graph, segment, n] returns exactly n or $Failed; UpTo[n] returns up to n; All returns all. Accepts a bare vertex sequence or an InfraSegment[{...}] wrapper. Option: Method (shared with FindSegment).";
...
FindBisectingHyperplane::usage = "FindBisectingHyperplane[graph, p1, p2] returns InfraPlane[{h}] for one inclusion-minimal subset of the bisector { v : d(p1, v) == d(p2, v) } whose removal disconnects p1 from p2. FindBisectingHyperplane[graph, p1, p2, n] / UpTo[n] / All controls multiplicity. FindBisectingHyperplane[graph, p1, p2, {lo, hi}, ...] widens the bisector to lo <= d(p1, v) - d(p2, v) <= hi.";
...
SelectPaths::usage = "SelectPaths[graph, paths, criterion] filters a path bundle by a path-space criterion (\"Central\" or \"Peripheral\", or a list of criteria folded left to right). Option: Method (\"Frechet\" (default) | \"Hausdorff\" | \"MeanFrechet\"). Accepts InfraSegment[paths_List]. Operator form SelectPaths[graph, criterion, opts][paths].";
SelectCycles::usage = "SelectCycles[graph, cycles, criterion] filters a cycle bundle (criterion: \"Central\", \"Peripheral\", \"ShortestCircumference\", \"LongestCircumference\", or a list). Option: Method (\"Frechet\" (default) | \"Hausdorff\" | \"MeanFrechet\"). Accepts InfraCircle[cycles_List]. Operator form SelectCycles[graph, criterion, opts][cycles].";
...
RadarCoordinates::usage = "RadarCoordinates[graph, vertex, basis] returns the radar coordinates of vertex (distance vector to basis). RadarCoordinates[graph, basis] returns the Association for all vertices.";
OrthogonalCoordinates::usage = "OrthogonalCoordinates[graph, axes, v] returns the layer-index tuple of v on each axis. OrthogonalCoordinates[graph, axes] returns the Association for all vertices. OrthogonalCoordinates[graph, c, v] auto-discovers axes through c and returns signed displacements. Axes accept InfraPoint[{...}] for multi-anchor centring. Options: \"Origin\", \"SelectCoordinate\" (First (default) | Last | Min | Max | Mean | Median | All | f).";
FindOrthogonalAxes::usage = "FindOrthogonalAxes[graph, n] returns n mutually well-separated longest paths (or $Failed). UpTo[n] returns up to n; All returns all. FindOrthogonalAxes[graph, v, n] / FindOrthogonalAxes[graph, InfraPoint[{...}], n] requires every axis to pass through v / one of the listed vertices. Options: \"AxisDistance\", \"MinLength\", \"MinSeparation\", \"AxisThickness\", \"RandomPick\".";
...
FindInfraScene::usage = "FindInfraScene[scene, graph] evaluates all construction steps and returns InfraInstance objects. FindInfraScene[scene, graph, n] caps to n steps. FindInfraScene[scene, graph, <|p -> v, ...|>] starts with pre-fixed bindings. Option: \"PruneProbability\".";
...
InfraPoint::usage = "InfraPoint[] is any vertex; InfraPoint[v] fixes a vertex; InfraPoint[\"Center\"] / InfraPoint[\"Periphery\"] selects from graph center / periphery; InfraPoint[origin, dist] selects vertices at given distance; InfraPoint[n] finds n mutually distant vertices. The wrapper form InfraPoint[{v1, v2, ...}] is the multi-realisation object returned by FindPoint and consumed by InfraSceneHighlight (forces point semantics). Accessors: [\"Realisations\"], [\"Length\"], [\"Expand\"], [\"First\"]; [[i]] returns a wrapped singleton. Nested wrappers auto-flatten.";
...
WolframRicci::usage = "WolframRicci[graph] returns Association[v -> mean_r R(v, r)] with the volume-comparison Ricci scalar averaged over the per-vertex valid radius range. WolframRicci[graph, {rmin, rmax}] restricts to a window. WolframRicci[graph, r] uses a single radius. Option: \"Dimension\" (Automatic (default), reads local dim off log-difference; integer d, fixed).";

(* ===================== Highlights / Viewers ===================== *)

InfraSceneHighlight::usage = "InfraSceneHighlight[g, multiObjects] renders a list of multi-objects diffusely on graph g. Each entry is auto-classified per representation; explicit wrappers (InfraPoint / InfraSegment / InfraLine / InfraShell / InfraPlane / InfraCircle / InfraRay / InfraPencil) force the intended semantics. Within one multi-object, vertex/edge intensity scales with overlap; across multi-objects, colors blend on shared elements. Default colour is by head; wrap as `entry -> color` to override. Options: \"OpacityRange\", \"ThicknessRange\", \"PointSizeRange\".";
InfraSceneViewer::usage = "InfraSceneViewer[scene, graph] is an interactive visualisation of an InfraScene on a graph. InfraSceneViewer[scene, graph, <|p -> v, ...|>] starts with pre-fixed bindings. Step slider controls the current step; \"Fix & advance\" pins the current instance; \"Reset\" clears bindings.";

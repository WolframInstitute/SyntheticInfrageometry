...
FindSegment::usage = "FindSegment[graph, p1, p2] returns the built-in single shortest path. With \"Select\" -> method, a pool of geodesics (capped by \"MaxShortestPaths\") is enumerated and filtered by a path-space-metric chooser (\"FrechetCentral\", \"HausdorffCentral\", \"EmbeddingClosest\", ...); the count argument (n / UpTo[n] / All) caps the filtered result. Without \"Select\", count is ignored.";
...
FindSphere::usage = "FindSphere[graph, c, r] returns the metric sphere of radius r around c -- the level surface { v : d(c, v) = r } -- as a single vertex set. FindSphere[graph, c, r, n] returns exactly n or $Failed; FindSphere[graph, c, r, UpTo[n]] returns up to n; FindSphere[graph, c, r, All] returns all. Options: Method (\"Metric\" (default) returns the level surface itself; \"SeparatingGraph\" returns minimal connected subsets of the level surface whose removal disconnects c from { v : d(c, v) > r }; \"SeparatingCycle\" further restricts to those forming a cycle in the level-surface subgraph -- the 2D-style spheres); \"Select\" sorts results in the multi-valued cases.";
...
FindBisector::usage = "FindBisector[graph, p1, p2] returns the full set of vertices equidistant from p1 and p2. FindBisector[graph, p1, p2, n] returns exactly n or $Failed; FindBisector[graph, p1, p2, UpTo[n]] returns up to n; FindBisector[graph, p1, p2, All] returns all bisector vertices (same as the no-count form). Accepts FindBisector[graph, {p1, p2}, ...] equivalently. Option: Method (\"Metric\" (default); \"Spectral\" and \"Resistance\" reserved, not yet implemented).";

...
FindMidpoint::usage = "FindMidpoint[graph, segment] returns a List with one unary InfraPoint[{v}] (the midpoint of segment). FindMidpoint[graph, p1, p2] returns one midpoint between p1 and p2 across all geodesics; with n / UpTo[n] / All controls multiplicity. Option: Method (\"Metric\" (default) | \"Tarski\" (synthetic from B and E) | \"Embedding\").";
...
CompleteEquilateralTriangle::usage = "CompleteEquilateralTriangle[graph, p1, p2] returns a List of unary InfraPoint[{c}] apex vertices equidistant from p1 and p2 at distance d(p1, p2) (Euclid I.1). With n / UpTo[n] / All controls multiplicity. Option: Method (\"Metric\" (default)).";
...
FindLine::usage = "FindLine[graph, p1, p2] returns {InfraSegment[{line}]} for one maximal geodesic extension through p1 and p2. FindLine[graph, p1, p2, n] returns a List of n unary InfraSegment[{line}] wrappers or $Failed; UpTo[n] returns up to n; All returns all. Options: \"Maximality\" (\"Extension\" (default) | \"Diameter\"), Method (\"ShortestPath\" (default), \"ShortestPathExtension\", \"CurvatureMinimizing\", \"Embedding\", \"Greedy\"), \"Pruning\" (caps prefix/suffix enumeration on \"ShortestPath\").";
FindParallel::usage = "FindParallel[graph, line, p] returns a List of unary InfraSegment[{line}] wrappers for parallels through p (parallel = constant distance to line), or $Failed. FindParallel[graph, line, p, n] / UpTo[n] / All controls multiplicity. Options: Method (\"ShortestPath\" (default) | \"Greedy\" | \"Embedding\"), \"Pruning\".";
FindPerpendicular::usage = "FindPerpendicular[graph, line, point] returns a List of unary InfraPoint[{foot}] wrappers for the feet of perpendiculars from point to line (Euclid I.12: isosceles base midpoint). FindPerpendicular[graph, line, point, n] / UpTo[n] / All controls multiplicity. Option: Method (\"Metric\" (default) | \"Embedding\").";
...
SegmentLineAngle::usage = "SegmentLineAngle[graph, p1, p2, line] (or SegmentLineAngle[graph, segment, line]) measures the distance from a segment endpoint to a line, given the other endpoint lies on the line. The name is historical -- the value is a length, not a normalised angle.";
...
FindPolylineSubdivision::usage = "FindPolylineSubdivision[graph, path] returns {InfraPolyline[{{seg1, ..., segk}}]} where the legs are the fewest geodesic InfraSegments whose knots are path-vertices and each leg is a shortest path since the previous knot. Option: \"MaxLength\" (Infinity (default) | numeric L) caps every leg's graph-length at L.";
PolylineQ::usage = "PolylineQ[graph, poly] tests whether poly = {seg1, ..., segk} of unary InfraSegment wrappers is a valid polyline in graph: every leg is a geodesic and consecutive legs share an endpoint. Accepts the wrapped form InfraPolyline[{...}] as well (AllTrue over realisations).";
...
FindRevolution::usage = "FindRevolution[graph, axis, profile] returns InfraObject[set] for the rotational vertex set around axis with the given radius profile. The axis is internally extended by +1 position on each side via ExtendSegment[..., \"Length\" -> 1] so the orthogonality test sees a one-step extension at each endpoint. The profile may be NumericQ (constant), a List of length |axis|, an Association vertex -> radius, or any callable applied to index 1..|axis|. Options: \"Form\" (\"Solid\" (default) | \"Surface\"); Method (\"Voronoi\" (default) -- closest extended position must be in the original axis range; \"PerpendicularBisector\" -- u included at position i iff d(u, v_{i-1}) == d(u, v_{i+1})).";
FindCylinder::usage = "FindCylinder[graph, axis, r] returns InfraObject[set] for the constant-radius rotational set around axis. Inherits FindRevolution options.";
FindCone::usage = "FindCone[graph, axis, slope] returns InfraObject[set] for the cone of given slope with apex at axis[[1]]: radii are slope * Range[0, Length[axis] - 1]. Option \"Apex\" (First (default) | Last) flips the apex end; inherits remaining options from FindRevolution.";
...
SelectPath::usage = "SelectPath[graph, paths] returns one path drawn from the bundle treated as a finite metric space in path-space. SelectPath[graph, paths, n] returns exactly n or $Failed; UpTo[n] returns up to n; All returns the whole pool. Options mirror FindPoint: \"From\" (All (default), \"Center\", \"Periphery\", \"MostVisited\", anchor -> spec, InfraSegment[{...}] -> spec, \"MinCurvature\" / \"MaxCurvature\" with optional nested {curv, agg} where curv is \"Forman\" | \"Wolfram\" | \"Ollivier\" and agg is \"Mean\" (default) | \"Total\" | \"Max\"), \"Distance\" (None (default), \"Max\", d, {dMin, dMax}), \"Metric\" (\"Hausdorff\" (default), \"Frechet\", \"MeanFrechet\"), \"MaxCliques\". Accepts InfraSegment and InfraRay wrappers; preserves the wrapper. Operator form SelectPath[graph, n, opts][paths].";
...
FindHomotopy::usage = "FindHomotopy[graph, p1, p2] returns {InfraHomotopy[{chain}]} exhibiting a chain of arc-swap moves taking walk p1 to walk p2 in graph, or {} if none is found within the search bounds. FindHomotopy[graph, p1, p2, n] returns a List of n unary InfraHomotopy[{chain}] wrappers or $Failed; UpTo[n] returns up to n; All returns all. Endpoints accept InfraSegment[{...}] for multi-anchor spread. Option: \"NullHomotopicCycles\" (integer k, list of integers, or list of vertex cycles -- declaring those cycles null-homotopic; integer k is shorthand for Range[2, k]; the degenerate length-2 case {a, b} is backtrack reduction a-b-a ~ a; default 3).";
FindNullHomotopy::usage = "FindNullHomotopy[graph, cycle] returns {InfraHomotopy[{chain}]} exhibiting a chain contracting cycle to its base vertex, or {} if cycle is not null-homotopic under the chosen \"NullHomotopicCycles\". With n / UpTo[n] / All controls multiplicity. Accepts InfraCircle[{...}] for multi-cycle spread. Inherits FindHomotopy options.";
HomotopicQ::usage = "HomotopicQ[graph, p1, p2] tests whether walks p1 and p2 are homotopic under the chosen \"NullHomotopicCycles\". Spreads over multi-realisation inputs as a Cartesian-AllTrue conjunction. Inherits FindHomotopy options.";
NullHomotopicQ::usage = "NullHomotopicQ[graph, cycle] tests whether cycle is null-homotopic under the chosen \"NullHomotopicCycles\" (contractible to a constant walk via arc-swap moves). Inherits FindHomotopy options.";
ReducePath::usage = "ReducePath[graph, path] applies length-decreasing arc-swap moves to path until no shortening move remains, returning a short representative of its homotopy class. Option: \"NullHomotopicCycles\" (default {2}, backtrack reduction only).";

...
(* rectification (ambo): vertices = edges of the map, two adjacent iff consecutive around
   a face -- the medial graph; takes {p, q} to (p.q.p.q) *)
RectifyMap[g_Graph] := First @ rectifyMapData @ mapData @ g;

(* truncation: each degree-q vertex becomes a q-gon, each p-gon face a 2p-gon;
   takes {p, q} to (q.2p.2p) *)
TruncateMap[g_Graph] := First @ truncateMapData @ mapData @ g;

(* dual: vertices = faces, two adjacent iff the faces share an edge; takes {p, q} to {q, p} *)
DualMap[g_Graph] := dualMapData @ mapData @ g;

(* expansion (cantellation) = ambo of ambo; takes {p, q} to (p.4.q.4) *)
ExpandMap[g_Graph] := First @ rectifyMapData @ rectifyMapData @ mapData @ g;

(* bevel (omnitruncation) = truncation of the rectification; takes {p, q} to (4.2p.2q) *)
BevelMap[g_Graph] := First @ truncateMapData @ rectifyMapData @ mapData @ g;
...
dualMapData[{g_, fcs_}] :=
  With[{fe = faceToEdges /@ fcs},
    SimpleGraph @ Graph @ Flatten @ Table[
      If[IntersectingQ[fe[[i]], fe[[j]]], UndirectedEdge[i, j], Nothing],
      {i, Length @ fe}, {j, i + 1, Length @ fe}]];
